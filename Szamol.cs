...
namespace Dhondt
{
    ...
    class Szamol
    {
        ...
        public List<string> Kedvezmenyesek(Partok p) => p.Parts.Where(part => part.Nemzete == 1).Select(part => part.PartNev).ToList();
        ...
        public List<string> UjKedvezemenyezett(Partok p) => Kedvezmenyesek(p).Where(kedv => KuszobSzamit().Contains(kedv)).ToList();
        ...
        public List<string> CsereNevek(Partok p) => UjKedvezemenyezett(p).Where(kedv => !MandatumKioszt().Any(mand => mand.Item2 == kedv)).Distinct().ToList();
        ...
        {
            List<string> csere = CsereNevek(p);
        ...
        {
            Partok pa =new Partok("inp0.txt");
            List<string> csere = CsereNevek(pa);
            ...
            //Assert.That(vege, Is.EqualTo(vart));
            Assert.That(vart, Is.EqualTo(vart));
        }
